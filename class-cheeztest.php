 ...
 * @phpcs:disable PHPCompatibility.FunctionUse.RemovedFunctions.create_functionDeprecated -- Batcache also has this dependency
 * @phpcs:disable WordPressVIPMinimum.Functions.RestrictedFunctions.create_function_create_function -- Batcache also has this dependency
 * @phpcs:disable WordPress.PHP.RestrictedPHPFunctions.create_function_create_function -- Batcache also has this dependency
 * @phpcs:disable Generic.PHP.DeprecatedFunctions.Deprecated -- Batcache also has this dependency
 *
 * @access  public
...
class CheezTest {
		...
		?>
			<script>
				( function(){
					...
					document.cookie = <?php echo wp_json_encode( $this->name ); ?> + '=' + <?php echo wp_json_encode( $this->group ); ?> + '; expires=' + <?php echo wp_json_encode( $this->expires ); ?> + '; path=/';
	...
	/**
	 * Helper func to get user's eligibility for any active test
	 *
	 ...
	 */
	public static function is_qualified_for( $name ) {
	...
	/**
	 * Helper func to get user's group assignment for any active test
	 *
	 * Returns 'group' property of static $active_tests collection
	 *
	 * @access public
	 * @param string $name Test name assigned during child initialization.
	 * @return string|bool Group name if user is eligible, false if not.
	 */
	public static function get_group_for( $name ) {
	...
	/**
	 * Helper func to check if user is in the specified group
	 *
	 * @access public
	 * @param string $name  Test name assigned during child initialization.
	 ...
	 */
	public static function is_in_group( $name, $group ) {
